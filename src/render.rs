// Derived from cargo-public-api's public-api crate. This is the heart and soul of the rustdoc JSON
// to `Token` conversion and the most complex part to maintain.

#![allow(clippy::unused_self)]
use crate::crate_set::{CrateSet, CrateTag};
...
use rustdoc_types::{
    Abi, AssocItemConstraint, AssocItemConstraintKind, Attribute, AttributeRepr, Constant,
...
/// struct.
pub struct RenderingContext<'a, 'c> {
    /// The original and unmodified rustdoc JSON, in deserialized form, of the
    /// crate being processed.
    pub crates: &'a CrateSet<'c>,

    /// Given a crate tag and a rustdoc JSON ID, keeps track of what public
    /// items that have this Id.
    pub id_to_items: &'a HashMap<(CrateTag, Id), Vec<&'a IntermediatePublicItem<'c>>>,

    /// Which crate's rustdoc JSON the item currently being rendered (and the
    /// IDs referenced by it) belongs to.
    pub tag: CrateTag,
}

impl<'a, 'c> RenderingContext<'a, 'c> {
    pub fn token_stream(&self, public_item: &IntermediatePublicItem<'c>) -> Vec<Token> {
        ...
        let inner_tokens = match &item.inner {
            ...
            ItemEnum::Macro(_definition) => {
                // TODO: _definition contains the whole definition, it would be really neat to get
                // out all possible ways to invoke it
            ...
            ItemEnum::Primitive(primitive) => {
                ...
                //   cargo run -- --rustdoc-json
                // ~/.rustup/toolchains/nightly-x86_64-unknown-linux-gnu/share/doc/rust/json/core.
                // json
                let mut output = pub_();
                output.extend([
                    Token::kind("type"),
        ...
        for id in fields {
            resolved_fields.push(
                if let Some(Item {
                    ...
                    .as_ref()
                    .and_then(|id| self.crates.get(self.tag).index.get(id))
                {
                    Some(type_)
                } else {
                    None
    ...
    fn render_nameable_item(&self, item: &NameableItem) -> (Vec<Token>, bool) {
        ...
        if let Some(name) = item.name() {
            // Some items (read: impls) do not have a name, so only push a name
            // if it exists
        ...
        output.extend(self.render_sequence(
            ...
            |(name, ty)| {
                self.simplified_self(name, ty).unwrap_or_else(|| {
                    ...
                    let _ = ignore_name; // parameter names are never included
                    output.extend(self.render_type(ty));
                    output
                })
            },
    ...
    fn render_resolved_path(&self, path: &Path) -> Vec<Token> {
        ...
        if let Some(item) = self.best_item_for_id(path.id) {
        ...
        } else if let Some(item) = self.crates.get(self.tag).paths.get(&path.id) {
            ...
            //
            // * The resolved path is for a public item inside a private mod (and thus effectively
    ...
    fn best_item_for_id(&self, id: Id) -> Option<&'a IntermediatePublicItem<'c>> {
        match self.id_to_items.get(&(self.tag, id)) {
            None => None,
            Some(items) => {
                items
                    .iter()
