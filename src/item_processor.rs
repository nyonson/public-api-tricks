...
pub struct ItemProcessor<'c> {
    /// The original and unmodified rustdoc JSON, in deserialized form, of the crate.
    crates: CrateSet<'c>,
...
impl<'c> ItemProcessor<'c> {
    pub(crate) fn new(crates: CrateSet<'c>) -> Self {
        ItemProcessor {
            crates,
    ...
    fn run(&mut self) {
        while let Some(unprocessed_item) = self.work_queue.pop_front() {
            if let Some(item) = self
                .crates
                .get_item(unprocessed_item.id.0, unprocessed_item.id.1)
            {
    ...
    fn process_use_glob_item(
        ...
        let tag = unprocessed_item.id.0;
        if let Some(Item {
            inner: ItemEnum::Module(Module { items, .. }),
            ..
        }) = use_
            .id
            .and_then(|id| self.get_item_if_not_in_path(tag, &unprocessed_item.parent_path, id))
        {
            for &item_id in items {
                self.add_to_work_queue(
                    unprocessed_item.parent_path.clone(),
                    unprocessed_item.parent,
                    (tag, item_id),
                );
            }
        } else if let Some((
        ...
        )) = self.resolve_external_use(use_)
        {
            // The glob re-export target is a module of an external crate
            // whose rustdoc JSON was provided. Inline its items.
            for &item_id in items {
                self.add_to_work_queue(
                    unprocessed_item.parent_path.clone(),
                    unprocessed_item.parent,
                    (external_tag, item_id),
                );
            }
        } else {
            self.process_item(
    ...
    fn process_use_item(
        ...
        item: &'c Item,
        use_: &'c Use,
        mut unprocessed_item: UnprocessedItem<'c>,
    ) {
        let tag = unprocessed_item.id.0;
        ...
        } else if let Some((external_tag, external_item)) = self.resolve_external_use(use_) {
            // The re-export target is an item of an external crate whose
            // rustdoc JSON was provided. Inline it. The logical parent is
            // still the module containing the `pub use`, so only the tag of
            // the item itself changes.
            actual_item = external_item;
            unprocessed_item.id.0 = external_tag;
        }

        self.process_item(unprocessed_item, actual_item, Some(use_.name.clone()));
    }

    /// Resolve the target of a `pub use` to an item of an external crate
    /// whose rustdoc JSON was provided via
    /// [`crate::Builder::external_rustdoc_json`].
    ///
    /// The target of a re-export of an external item is not present in the
    /// rustdoc JSON `index`, but the `paths` map tells us which external
    /// crate it comes from and its full path within that crate. The path is
    /// resolved to an item by walking the external crate's module tree.
    fn resolve_external_use(&self, use_: &'c Use) -> Option<(CrateTag, &'c Item)> {
        let id = use_.id?;
        let summary = self.crates.main().paths.get(&id)?;
        let tag = self.crates.external_tag(summary.crate_id)?;
        let crate_ = self.crates.get(tag);
        let id = resolve_path_in_crate(crate_, &summary.path)?;
    ...
    fn process_item_for_type(
        ...
        let children = self.children_for_item(tag, item);
        let impls = impls_for_item(item).into_iter().flatten();

        for &id in &children {
            self.add_to_work_queue(finished_item.path().into(), Some((tag, item.id)), (tag, id));
        }
        ...
        for &id in impls {
            ...
            self.add_to_work_queue(path, Some((tag, item.id)), (tag, id));
        }
    ...
    fn children_for_item(&mut self, tag: CrateTag, item: &'c Item) -> Vec<Id> {
        match &item.inner {
            ...
            ItemEnum::Impl(impl_) => {
                ...
                if let Some(trait_path) = &impl_.trait_ {
                    let explicit_names: Vec<String> = impl_
                        .items
                        .iter()
                        .filter_map(|&id| self.crates.get_item(tag, id))
                        .filter_map(|item| item.name.clone())
                        .collect();

                    if let Some(trait_item) = self.crates.get_item(tag, trait_path.id)
                        && let ItemEnum::Trait(trait_) = &trait_item.inner
                    {
                        for &trait_child_id in &trait_.items {
                            if let Some(trait_child) = self.crates.get_item(tag, trait_child_id)
    ...
    // real-world code.
    //
    // Keys are qualified with the crate the Id belongs to, because rustdoc
    // JSON Ids are only unique per crate.
    fn id_to_items(&self) -> HashMap<(CrateTag, Id), Vec<&IntermediatePublicItem<'_>>> {
        let mut id_to_items: HashMap<(CrateTag, Id), Vec<&IntermediatePublicItem<'_>>> =
            HashMap::new();
        for finished_item in &self.output {
            id_to_items
                .entry((finished_item.tag(), finished_item.item().id))
                .or_default()
                .push(finished_item);
        }
        id_to_items
...
/// Resolve a path (e.g. `["some_crate", "some_module", "SomeStruct"]`) to an
/// item of a crate by walking its module tree. The first path component is
/// the name of the crate itself, i.e. the root module.
fn resolve_path_in_crate(crate_: &Crate, path: &[String]) -> Option<Id> {
    let mut id = crate_.root;
    for segment in &path[1..] {
        ...
        id = module
            ...
            .iter()
            .find(|&&child_id| {
                crate_.index.get(&child_id).and_then(|i| i.name.as_deref())
                    == Some(segment.as_str())
            })
            .copied()?;
    }
...
impl<'c> UnprocessedItem<'c> {
    ...
    fn finish(
        ...
        // Done
        IntermediatePublicItem::new(path, self.parent, (self.id.0, item.id))
    }
}
...
pub(crate) fn public_api_in_crate(
    ...
    let mut item_processor = ItemProcessor::new(CrateSet::new(crate_, externals, artifact_paths));
    item_processor.add_to_work_queue(vec![], None, (CrateTag::Main, crate_.root));
    item_processor.run();

    let id_to_items = item_processor.id_to_items();

    item_processor
        .output
        .iter()
        .map(|item| {
            // Each item renders with the rustdoc JSON of the crate it belongs
            // to.
            let context = RenderingContext {
                crates: &item_processor.crates,
                id_to_items: &id_to_items,
                tag: item.tag(),
            };
            PublicItem::from_intermediate_public_item(&context, item)
        })
        .collect()
}
